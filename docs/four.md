...
Previous architectures have their problems. A clustering model could understand that if someone's purchased a belt, let's recommend them another belt since they clearly like belts. Or, by defining sets of complementary products, another model could understand that if they've already purchased a belt, perhaps they'd like a pair of trousers to go with it. There's ways of combining these two approaches, using architectures like a feature injected BERT4Rec, which is also built off of the transformer architecture (like modern LLMs). 

However, all of these approaches can be summed up by - I found this trend in the data, and I adapted this model to handle it. The motivation behind our use of LLMs is that they should not only be able to find the trends in the data themselves, but be able to leverage it's own understanding of the world in its prediction. It takes the onus off of the data engineer to find the trends, and allows the model to find them itself.

Later, we test this hypothesis - for all the trends we ourselves find in the data, we look in the predictions made by the LLM for those same trends. If they are present, we assume that it has found further trends that we ourselves have not. 
